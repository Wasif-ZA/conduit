...
### MCP transport: Option 2 (mcp-remote stdio bridge) locked; Option 1 attempted 2026-04-28, blocked by spike infra
- **Date**: 2026-04-26 (Sun admin night, ~21:30 AEST), updated 2026-04-28T13:30+10:00
- **Decision**: Lock **Option 2** (`mcp-remote` stdio bridge in Claude Desktop config) as the transport for the Wed 2026-04-29 demo and the documented default beyond. Option 1 (streamable-HTTP `url`+`headers` config) was attempted Tue 2026-04-28 AM and could not be validated; deferred indefinitely. Re-evaluate against the real Conduit endpoint when it ships, not against the spike.
- **Why**: Sunday night's spike validated everything below the transport layer end-to-end. `mcp-handler@1.0.4` + `withMcpAuth` build clean on Next.js 16, the Vercel preview deployed cleanly. The remaining gate (does Claude Desktop accept `url`+`headers`?) was blocked Sunday night by Vercel's default Deployment Protection wall, which gates preview URLs behind Vercel SSO. Disabling that takes one click in Vercel settings, but the user was past their useful focus window. Option 2 is the universal fallback shape every MCP server documents; locking it Sunday night lets Mon's runtime work proceed with a confirmed-working client config. Option 1 is purely a client-side UX upgrade, the server code is identical.
- **2026-04-28 attempt outcome**:
  - Vercel Deployment Protection toggled off (took two passes; preview deployments have a separate toggle from production, easy to miss).
  - Spike `/api/mcp` reachable but unstable. Alternates between `HTTP 500 + empty body` (function crashes outright) and `HTTP 406 + malformed body` where the response body is a decimal-encoded byte array (e.g. literal text `123,34,106,115,...` rather than the JSON `{"jsonrpc":...}`). Smells like a `Uint8Array` → default `toString` coercion somewhere in the response pipeline; likely a Next.js 16.2.4 / mcp-handler 1.0.4 / Node 24 incompatibility.
  - Without a stable response, could not verify whether Claude Desktop's `url`+`headers` config triggers an outbound connection. Cowork mode in Claude Desktop doesn't surface user-config MCP servers, and the install on this machine has no Settings → Developer/Connectors panel to inspect, so verification routes were thin.
  - Stopped debugging; the spike is throwaway code (per the comment header in `app/api/mcp/route.ts`), not worth fixing. Sunday-overhang closed.
- **Status**: locked (Option 2)
- **Spike artefacts**: throwaway scratch repo `03 Projects/conduit-transport-spike/` (Next.js 16 + `mcp-handler@1.0.4` + `withMcpAuth` + a single `echo` tool). NOT to be merged into the real `conduit/` repo, NOT in git. Has the Next.js / mcp-handler instability noted above. Discard or archive; do not use as a reference implementation.
